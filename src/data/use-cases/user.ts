...
import type { IUserBuilder, IHashed, IUserRepository, ICryptography } from '@/data/protocols';
...
export class User implements IUser, IAuthentication {
  ...
  async create(userDto: Omit<SignUpUserDto, 'passwordConfirmation'>): Promise<UserOutputDto | null> {
    ...
    const buildUser = this?.userBuilder?.buildUserBasicInfo({ ...userDto, password: hashedPassword }) as SignUpUserDto;
    return this.userRepository.create(buildUser);
  }
  ...
  async updateUserPassword(
    redefineUserPasswordDto: Omit<UpdateUserPasswordDto, 'passwordConfirmation'>,
  ): Promise<void> {
    const { password } = redefineUserPasswordDto;
    const hashedPassword = await this.hashed.hash(password);
    await this.userRepository.updateUserPassword({ ...redefineUserPasswordDto, password: hashedPassword });
  }
  ...
  async getAllUsers(filterUserDto: FilterUserDto): Promise<GetUserOutputDto[]> {
    return this.userRepository.getAllUsers(filterUserDto);
  }
}
